...
# MAGIC %md The purpose of this notebook is to introduce the Clickstream Propensity solution acclerator and to provide access to configuration information for the notebooks supporting it. This notebook was developed on a **Databricks ML 12.1** cluster.
...
# MAGIC 
# MAGIC In this solution accelerator, we leverage event data from an electronics ecommerce sit to demonstrate how such data may be used to estimate these propensities.  We first use a historical backlog of these data to train a model.  We then implement a real-time stream to calculate features supporting inference with each page clicked.  You can think of the accelerator as divided into these two parts where in the first we focus on understanding the data and how we might approach the Data Science portion of our work.  In the second, the focus is on the mechanics of building a real-time inference layer.  Before diving into those parts (identify by the number associated with different notebooks, be sure to run notebook *CS 0b* to stage the data required for both parts.)
# MAGIC 
# MAGIC **NOTE** This accelerator uses the newly released Databricks Model Serving capability which is avaiable in only select regions.  Please consult the feature documentation to determine if your workspace is in a [supported region](https://learn.microsoft.com/en-us/azure/databricks/machine-learning/model-serving/) before deploying the accelerator.

# COMMAND ----------

# MAGIC %md ###Understanding Part 1: Model Training
# MAGIC 
# MAGIC Part 1 of this accelerator is comprised of two notebooks named *CS 1a* and *CS 1b*.  In the first of these notebooks, we generate point-in-time features from historical clickstream data that we've accumulated.  In setting up the data, we pretend as if that data has been accumulated through a workflow organized around a medallion architecture.  
# MAGIC 
# MAGIC The medallion architecture, explained in more depth in notebook *CS 1a*, divides our data processing workflow into three high-level stages.  
# MAGIC </p>
# MAGIC 
...
# DBTITLE 1,Storage (see notebook CS 0b for more info)
# mount point name
config['mount_point'] ='/mnt/clickstream'
...
# DBTITLE 1,Streaming Ingest (see notebook CS 2a for more info)
# azure event hub sas policy connection string
event_hub_sas_policy_connection_string = 'Endpoint=sb://brysmi-clickstream.servicebus.windows.net/;SharedAccessKeyName=send_and_receive;SharedAccessKey=eJj1x0rxuEcWxtyn0FRQauD+H1IsFKpUbTLWwE5YkFU=;EntityPath=electronics'
...
# DBTITLE 1,Online Feature Store (see notebook CS 2b for more info)
# azure cosmosdb uri
config['cosmosdb_uri'] = 'https://brysmi-click2.documents.azure.com:443/'
...
# DBTITLE 1,Model (see notebooks CS 1b & 2d for more info)
